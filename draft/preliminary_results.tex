...
\date{July 4, 2026 --- draft v0.3 (companion to the ``Why Rosenzweig--Porter?'' program)}
...
post-designable via a validated second-order control law; the
Mindlin thickness sweep confirms the thick-plate prediction at $+5.4\sigma$
(GOE-ward); and at the registered $N=2048$ truncated-operator rung the
non-ergodic \emph{fractal RP phase develops} ($D_2=0.42$--$0.50$) with flat
$D_q$ excluding the power-law-banded rival --- while the preregistered
dimension value ($0.76\pm0.15$) is not confirmed. All experiments, gates,
and raw data are reproducible from the repository.
\end{abstract}
...
power-law-banded rival calibration $0.20$--$0.28$). The non-ergodic
\emph{fractal (RP) phase develops at the registered ladder, with the PBRM
rival excluded}; the P12 preregistered dimension value ($0.76\pm0.15$) is
\emph{not} confirmed (measured $\approx0.46$). The truncated-operator and
true-operator protocols disagree exactly where the T1 audit predicted they
could; their reconciliation at matching $N$ (a $C^0$ interior-penalty
discretization; the Argyris uniform-mesh ceiling is measured at
$h\approx0.008$ and skfem's Lagrange elements lack second derivatives) is
...
and the eigenvector program (E4, E8) shows the hierarchy is not only
spectral: adapted boundaries carry sparse, $N$-independent eigenvector
spreads (rectangle $\approx$ beam products; ellipse $\approx$ 2.5 SS modes),
while unadapted boundaries carry genuinely scaling eigenvectors
($D_2\approx0.2$--$0.5$) --- the RP-candidate regime living exactly where
the spacing statistics are strongest. What the data refine is the
\emph{selector}: not corners, not curvature, but whether the boundary is
adapted to a separable structure of the fourth-order operator. The
effective coupling is also material-tunable ($\lambda(\nu)$, E7) and
post-designable with a quantitative control law (E6) --- both usable knobs
for the experimental program. Whether the scaling eigenvectors of the
unadapted tier become RP-fractal (flat $D_q$) or PBRM-multifractal at
higher rungs is the single most consequential open computation.

\section{Pending registered tests}

All tests executable with the present machinery are now done (E1--E11).
Registered follow-ups: reconciliation of the truncated-operator and
true-operator Gap~A protocols at matching $N\sim1024$--$2048$ (requires a
$C^0$ interior-penalty discretization with custom reference hessians; the
Argyris ceiling is measured at $h\approx0.008$); the sector at the
published study's exact angles and mode counts; a full-rank rerun of the E6
dichotomy ($\sim$200 quartets); aspect-ratio robustness sweeps; and
