...
\date{July 4, 2026 --- draft v0.2 (companion to the ``Why Rosenzweig--Porter?'' program)}
...
In execution at the time of this draft revision: \textbf{E10} (annulus, the
second exactly-reduced Poisson control, via a $4\times4$ $J/Y/I/K$
free-edge determinant with FEM cross-validation) and \textbf{E11}
(Mindlin--Reissner thickness sweep; formulation validated against the
certified Kirchhoff spectrum in the thin limit). Pending, registered in the
research plan: Gap~A at $N\sim2048$ via a $C^0$ interior-penalty
discretization (the Argyris uniform-mesh ceiling is measured at
$h\approx0.008$) --- this adjudicates both the RP-phase question and the
$D_q$ separator; the sector at the published study's exact angles and mode
counts; a full-rank rerun of the E6 dichotomy; and comparison against the
