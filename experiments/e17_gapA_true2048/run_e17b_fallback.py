#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""E17b -- FALLBACK (contingency, registered 2026-07-07 while E17 runs):
COVERAGE-LIMITED configuration of E17 -- n_modes 3600, ladder to
N = 1024 -- ~60% of the memory and ~40% of the compute. To be run ONLY if
E17 dies on the memory ceiling. The preregistered E17 readings apply with
references recalibrated to the N = 1024 rung: E9 truncated value
interpolated from its 256->2048 ladder (0.069); onset threshold at the
log-midpoint of the flat (0.174) and truncated references (<= 0.10);
artifact threshold unchanged (>= 0.14)."""
import json
import os
import time
...
E9_SINE_TOP = 0.069  # E9 truncated ladder interpolated at N = 1024 (0.123 -> 0.052 over 256 -> 2048)
...
def main():
    ...
    md = ["# E17 -- Gap A true-operator windows at N = 1024-2048 (RESULTS)\n",
          ...
          f"truncated sine at N = 1024 (interpolated): {E9_SINE_TOP}; E4/E14 flat level: "
          f"~{FLAT_SINE}.\n"]
    for g in ("G1", "G2", "G3"):
    ...
    if top < max(cfg["ladder"]):
        ...
        verdict = ("PROTOCOL ARTIFACT THROUGH THE FULL REGISTERED RANGE: "
                   "true-operator windows stay sparse to the top covered rung; "
        ...
        verdict = (f"INTERMEDIATE: between references at the top rung "
