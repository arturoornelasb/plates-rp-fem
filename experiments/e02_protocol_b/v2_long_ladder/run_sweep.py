...
CFG = dict(
    ...
    b=1.0 / 1.6189043236,
    nu=0.33,
    # v2 (long ladder): production mesh 128x80, ~200 levels/sector, reduced
    # kappa grid (endpoints + transition core). Purpose: raise the transition
    # amplitude above the preregistered 3 sigma (v1 measured 2.1 sigma at
    # ~100 levels/sector). G1 (exact SSSS on the production mesh) remains the
    # primary accuracy gate; G2 vs the coarser 96x60 is a consistency check.
    mesh=(128, 80),
    mesh_check=(96, 60),
    n_modes=800,                       # elastic modes stored per kappa
    kappas=[0.0, 1e4, 3e5, 1e6, 3e6, 1e8, 1e10],
    sigma=-10.0,
    spacing_frac=0.1,
...
def main():
    ...
    print(f"[G2] vs {cfg['mesh_check'][0]}x{cfg['mesh_check'][1]} "
          f"(rigid {n_rigid_c}, max {rigid_max_c:.1e}): "
